...
Parser::terminal_symbol_t  Parser::lexer( char c_ ) const {
    switch( c_ ) {
        ...
        case '(':  return terminal_symbol_t::TS_PARE;
        case ')':  return terminal_symbol_t::TS_PARD;
...
bool Parser::accept( terminal_symbol_t c_ ) {
    ...
    if (!end_input() && lexer( *m_it_curr_symb ) == c_  ) {
        next_symbol();
        return true;
    }
    return false;
...
void Parser::skip_ws( void ) {
    ...
    while (!end_input() && std::isspace( *m_it_curr_symb ) )
        next_symbol();
}
...
bool Parser::expression( void ) {
    ...
    while( m_result.type == ResultType::OK ) {
        ...
        //else if( accept( Parser::terminal_symbol_t::TS_PARE ) ){
            // Stores the "(" token in the list.
        //    m_tk_list.emplace_back( Token{ "(", Token::token_t::OPERATOR } );
        //}
        //else if( accept( Parser::terminal_symbol_t::TS_PARD ) ){
            // Stores the ")" token in the list.
        //    m_tk_list.emplace_back( Token{ ")", Token::token_t::OPERATOR } );
        //}
        else break;
        ...
        if (!term() && m_result.type == ResultType::ILL_FORMED_INTEGER ) {
...
bool Parser::term( void ) {
    ...
    if ( integer() ) {
        ...
        if ( token_value < std::numeric_limits< required_int_type >::min() ||
    ...
    else if( accept( terminal_symbol_t::TS_PARE ) ){
        m_tk_list.emplace_back( Token{ "(", Token::token_t::OPERATOR } );
        if(expression()){
            if(accept( terminal_symbol_t::TS_PARD )){
                m_tk_list.emplace_back( Token{ ")", Token::token_t::OPERATOR } );
            }else{
                m_result = ResultType{ ResultType::MISSING_CLOSING, token_location() };
            }
        }else{
            m_result = ResultType{ ResultType::MISSING_EXP, token_location() };
        }
    }
    else {
...
bool Parser::digit( void ) {
    return ( accept( terminal_symbol_t::TS_ZERO ) || digit_excl_zero() ) ? true : false;
}
...
Parser::ResultType  Parser::parse_and_tokenize( std::string e_ ) {
    ...
    if ( end_input() ) { // Premature end?
        ...
        if ( expression() ) {
            ...
            if (!end_input() ) {// Is there still any residual symbol left in the string?
